...
ENDPOINT   = "/run-tests/Flyway/scenario2" 
ITERATIONS = 2
...
def plot_results(migration_times):
    ...
    plt.title(f"Flyway Scenario2 ({ITERATIONS} runs): Running Avg Migration Latency")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(AVG_PNG)
    ...
    plt.title(f"Flyway Scenario2 ({ITERATIONS} runs): Migration Latency Distribution")
    plt.grid(True)
    plt.tight_layout()
