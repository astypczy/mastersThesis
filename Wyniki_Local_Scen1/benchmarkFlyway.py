...
ENDPOINT   = "/run-tests/Flyway/scenario1" 
ITERATIONS = 2

RAW_CSV     = "results_Flyway_scenario1.csv"
SUMMARY_CSV = "summary_Flyway_scenario1.csv"
AVG_PNG     = "avg_migration_latency_Flyway_scenario1.png"
HIST_PNG    = "migration_latency_histogram_Flyway_scenario1.png"
...
def measure_and_record(n):
    ...
    with open(RAW_CSV, "a", newline="") as f:
        ...
        for i in range(1, n+1):
            ...
            url = f"{BASE_URL}{ENDPOINT}"  # single-run
...
def plot_results(migration_times):
    ...
    plt.title(f"Flyway Scenario1 ({ITERATIONS} runs): Running Avg Migration Latency")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(AVG_PNG)
    ...
    plt.title(f"Flyway Scenario1 ({ITERATIONS} runs): Migration Latency Distribution")
    plt.grid(True)
    plt.tight_layout()
...
def main():
    print(f"Starting Flyway benchmark: {ITERATIONS} runs of {ENDPOINT}")
