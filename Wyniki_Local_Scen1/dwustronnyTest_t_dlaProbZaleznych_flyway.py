...
# 2. Przygotuj pary Flyway vs Flyway-R według context i repliki
#    – nadaj replikę wewnątrz każdej grupy context
df_fw_mig = (
    df[(df['tool']=='Flyway') & df['identifier'].str.contains('.fwd.')]
      .assign(rep=lambda d: d.groupby('context').cumcount()+1)
      .set_index(['context','rep'])['migration_ns']
)
df_fw_rb = (
    df[(df['tool']=='Flyway') & df['identifier'].str.contains('.rbk.')]
      .assign(rep=lambda d: d.groupby('context').cumcount()+1)
