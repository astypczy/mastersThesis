...
ENDPOINT   = "/run-tests/Liquibase/scenario1" 
ITERATIONS = 2

RAW_CSV     = "results_liquibase_scenario1.csv"
SUMMARY_CSV = "summary_liquibase_scenario1.csv"
AVG_PNG     = "avg_migration_latency_liquibase_scenario1.png"
HIST_PNG    = "migration_latency_histogram_liquibase_scenario1.png"
...
def plot_results(migration_times):
    ...
    plt.title(f"Liquibase Scenario1 ({ITERATIONS} runs): Running Avg Migration Latency")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(AVG_PNG)
    ...
    plt.title(f"Liquibase Scenario1 ({ITERATIONS} runs): Migration Latency Distribution")
    plt.grid(True)
    plt.tight_layout()
